...
const getFormattedCredentials = () => {
    return {
        phone: credentials.phone.replaceAll('(+84)','').replaceAll(' ', '').replace('(', '').replace(')', '').replaceAll('-', ''),
        login_code: credentials.login_code
    }
}

const handleLogin = () => {
    axios.post('http://localhost:8082/api/login', getFormattedCredentials())
        .then((response) => {
...
const handleVerification = () => {
    axios.post('http://localhost:8082/api/login/verify', getFormattedCredentials())
        .then((response) => {
